...
GameObject* ChatHandler::GetNearbyGameObject()
{
    ...
    GameObject* obj = NULL;
    SurgeCore::NearestGameObjectCheck check(*pl);
    SurgeCore::GameObjectLastSearcher<SurgeCore::NearestGameObjectCheck> searcher(pl, obj, check);
...
GameObject* ChatHandler::GetObjectGlobalyWithGuidOrNearWithDbGuid(uint32 lowguid, uint32 entry)
{
    ...
    if (!obj && sObjectMgr->GetGOData(lowguid))                   // guid is DB guid of object
    {
        ...
        CellCoord p(SurgeCore::ComputeCellCoord(pl->GetPositionX(), pl->GetPositionY()));
        Cell cell(p);

        SurgeCore::GameObjectWithDbGUIDCheck go_check(*pl, lowguid);
        SurgeCore::GameObjectSearcher<SurgeCore::GameObjectWithDbGUIDCheck> checker(pl, obj, go_check);

        TypeContainerVisitor<SurgeCore::GameObjectSearcher<SurgeCore::GameObjectWithDbGUIDCheck>, GridTypeMapContainer > object_checker(checker);
