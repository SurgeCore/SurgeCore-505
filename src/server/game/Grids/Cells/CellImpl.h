...
inline CellArea Cell::CalculateCellArea(float x, float y, float radius)
{
    if (radius <= 0.0f)
    {
        CellCoord center = SurgeCore::ComputeCellCoord(x, y).normalize();
        return CellArea(center, center);
    }

    CellCoord centerX = SurgeCore::ComputeCellCoord(x - radius, y - radius).normalize();
    CellCoord centerY = SurgeCore::ComputeCellCoord(x + radius, y + radius).normalize();
