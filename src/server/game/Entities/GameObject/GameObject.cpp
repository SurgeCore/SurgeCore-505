        ...
        case GO_READY:
        {
            ...
            if (isSpawned())
            {
                ...
                if (goInfo->type == GAMEOBJECT_TYPE_TRAP)
                {
                    ...
                    {
                        SurgeCore::AnyUnfriendlyNoTotemUnitInObjectRangeCheck checker(this, owner, radius);
                        SurgeCore::UnitSearcher<SurgeCore::AnyUnfriendlyNoTotemUnitInObjectRangeCheck> searcher(this, ok, checker);
                    ...
                    else                                        // environmental trap
                    {
                        ...
                        // affect only players
                        Player* player = NULL;
                        SurgeCore::AnyPlayerInObjectRangeCheck checker(this, radius);
                        SurgeCore::PlayerSearcher<SurgeCore::AnyPlayerInObjectRangeCheck> searcher(this, player, checker);
...
void GameObject::TriggeringLinkedGameObject(uint32 trapEntry, Unit* target)
{
    ...
    GameObject* trapGO = NULL;
    {
        ...
        SurgeCore::NearestGameObjectEntryInObjectRangeCheck go_check(*target, trapEntry, range);
        SurgeCore::GameObjectLastSearcher<SurgeCore::NearestGameObjectEntryInObjectRangeCheck> checker(this, trapGO, go_check);

        TypeContainerVisitor<SurgeCore::GameObjectLastSearcher<SurgeCore::NearestGameObjectEntryInObjectRangeCheck>, GridTypeMapContainer > object_checker(checker);
...
GameObject* GameObject::LookupFishingHoleAround(float range)
{
    ...
    CellCoord p(SurgeCore::ComputeCellCoord(GetPositionX(), GetPositionY()));
    Cell cell(p);
    SurgeCore::NearestGameObjectFishingHole u_check(*this, range);
    SurgeCore::GameObjectSearcher<SurgeCore::NearestGameObjectFishingHole> checker(this, ok, u_check);

    TypeContainerVisitor<SurgeCore::GameObjectSearcher<SurgeCore::NearestGameObjectFishingHole>, GridTypeMapContainer > grid_object_checker(checker);
            ...
            if (GetUniqueUseCount() == info->summoningRitual.reqParticipants)
            {
                ...
                if (info->summoningRitual.casterTargetSpell && info->summoningRitual.casterTargetSpell != 1) // No idea why this field is a bool in some cases
                    for (uint32 i = 0; i < info->summoningRitual.casterTargetSpellTargets; i++)
                        // m_unique_users can contain only player GUIDs
                        if (Player* target = ObjectAccessor::GetPlayer(*this, SurgeCore::Containers::SelectRandomContainerElement(m_unique_users)))
