...
uint32 Unit::DealDamage(Unit* victim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellInfo const* spellProto, bool durabilityLoss)
{
        ...
        && spellProto->Id != 130651 && spellProto->Id != 117993) // Don't triggered by Zen Sphere, Spinning Crane Kick, Chi Wave, Chi Burst and Chi Torpedo
    {
        ...
        if (targetList.size() > 1)
        {
            ...
            targetList.sort(SurgeCore::HealthPctOrderPred());
            targetList.resize(1);
        }
...
void Unit::CalcAbsorbResist(Unit* victim, SpellSchoolMask schoolMask, DamageEffectType damagetype, uint32 const damage, uint32 *absorb, uint32 *resist, SpellInfo const* spellInfo)
{
    ...
    vSchoolAbsorbCopy.sort(SurgeCore::AbsorbAuraOrderPred());
...
int32 Unit::DealHeal(Unit* victim, uint32 addhealth, SpellInfo const* spellProto)
{
    ...
    else if (unit && unit->GetTypeId() == TYPEID_PLAYER && unit->HasAura(117907) && unit->getLevel() >= 80 && addhealth > 0 && spellProto && spellProto->Id != 124041)
    {
        ...
        if (roll_chance_f(Mastery * scaling))
        {
            ...
            SurgeCore::AnyFriendlyUnitInObjectRangeCheck u_check(unit, unit, 6.0f);
            SurgeCore::UnitListSearcher<SurgeCore::AnyFriendlyUnitInObjectRangeCheck> searcher(unit, targetList, u_check);
...
void Unit::GetAttackableUnitListInRange(std::list<Unit*> &list, float fMaxSearchRange) const
{
    ...
    SurgeCore::AnyUnitInObjectRangeCheck u_check(this, fMaxSearchRange);
    SurgeCore::UnitListSearcher<SurgeCore::AnyUnitInObjectRangeCheck> searcher(this, list, u_check);
...
Unit* Unit::SelectNearbyTarget(Unit* exclude, float dist) const
{
    ...
    SurgeCore::AnyUnfriendlyUnitInObjectRangeCheck u_check(this, this, dist);
    SurgeCore::UnitListSearcher<SurgeCore::AnyUnfriendlyUnitInObjectRangeCheck> searcher(this, targets, u_check);
    ...
    // select random
    return SurgeCore::Containers::SelectRandomContainerElement(targets);
}

Unit* Unit::SelectNearbyAlly(Unit* exclude, float dist) const
{
    std::list<Unit*> targets;
    SurgeCore::AnyFriendlyUnitInObjectRangeCheck u_check(this, this, dist);
    SurgeCore::UnitListSearcher<SurgeCore::AnyFriendlyUnitInObjectRangeCheck> searcher(this, targets, u_check);
    VisitNearbyObject(dist, searcher);
...
public:
    ...
    {
        SurgeCore::AIRelocationNotifier notifier(m_owner);
...
bool Unit::UpdatePosition(float x, float y, float z, float orientation, bool teleport)
{
    ...
    if (!SurgeCore::IsValidMapCoord(x, y, z, orientation))
    {
