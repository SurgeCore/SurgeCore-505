...
void AchievementMgr<T>::SendAchievementEarned(AchievementEntry const* achievement) const
{
    ...
    {
        SurgeCore::AchievementChatBuilder say_builder(*GetOwner(), CHAT_MSG_GUILD_ACHIEVEMENT, LANG_ACHIEVEMENT_EARNED, achievement->ID);
        SurgeCore::LocalizedPacketDo<SurgeCore::AchievementChatBuilder> say_do(say_builder);
    ...
    {
        CellCoord p = SurgeCore::ComputeCellCoord(GetOwner()->GetPositionX(), GetOwner()->GetPositionY());

        Cell cell(p);
        cell.SetNoCreate();

        SurgeCore::AchievementChatBuilder say_builder(*GetOwner(), CHAT_MSG_ACHIEVEMENT, LANG_ACHIEVEMENT_EARNED, achievement->ID);
        SurgeCore::LocalizedPacketDo<SurgeCore::AchievementChatBuilder> say_do(say_builder);
        SurgeCore::PlayerDistWorker<SurgeCore::LocalizedPacketDo<SurgeCore::AchievementChatBuilder> > say_worker(GetOwner(), sWorld->getFloatConfig(CONFIG_LISTEN_RANGE_SAY), say_do);
        TypeContainerVisitor<SurgeCore::PlayerDistWorker<SurgeCore::LocalizedPacketDo<SurgeCore::AchievementChatBuilder> >, WorldTypeMapContainer > message(say_worker);
