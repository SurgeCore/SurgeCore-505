...
inline GameObject* Map::_FindGameObject(WorldObject* searchObject, uint32 guid) const
{
    ...
    CellCoord p(SurgeCore::ComputeCellCoord(searchObject->GetPositionX(), searchObject->GetPositionY()));
    Cell cell(p);

    SurgeCore::GameObjectWithDbGUIDCheck goCheck(*searchObject, guid);
    SurgeCore::GameObjectSearcher<SurgeCore::GameObjectWithDbGUIDCheck> checker(searchObject, gameobject, goCheck);

    TypeContainerVisitor<SurgeCore::GameObjectSearcher<SurgeCore::GameObjectWithDbGUIDCheck>, GridTypeMapContainer > objectChecker(checker);
        ...
        switch (step.script->command)
        {
            ...
            case SCRIPT_COMMAND_CALLSCRIPT_TO_UNIT:
            {
                ...
                if (source) //using grid searcher
                {
                    ...
                    CellCoord p(SurgeCore::ComputeCellCoord(wSource->GetPositionX(), wSource->GetPositionY()));
                    Cell cell(p);

                    SurgeCore::CreatureWithDbGUIDCheck target_check(wSource, step.script->CallScript.CreatureEntry);
                    SurgeCore::CreatureSearcher<SurgeCore::CreatureWithDbGUIDCheck> checker(wSource, cTarget, target_check);

                    TypeContainerVisitor<SurgeCore::CreatureSearcher <SurgeCore::CreatureWithDbGUIDCheck>, GridTypeMapContainer > unit_checker(checker);
