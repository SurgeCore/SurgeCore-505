...
ObjectList* SmartScript::GetWorldObjectsInDist(float dist)
{
    ...
    if (obj)
    {
        SurgeCore::AllWorldObjectsInRange u_check(obj, dist);
        SurgeCore::WorldObjectListSearcher<SurgeCore::AllWorldObjectsInRange> searcher(obj, *targets, u_check);
...
Unit* SmartScript::DoSelectLowestHpFriendly(float range, uint32 MinHPDiff)
{
    ...
    CellCoord p(SurgeCore::ComputeCellCoord(me->GetPositionX(), me->GetPositionY()));
    Cell cell(p);
    cell.SetNoCreate();

    Unit* unit = NULL;

    SurgeCore::MostHPMissingInRange u_check(me, range, MinHPDiff);
    SurgeCore::UnitLastSearcher<SurgeCore::MostHPMissingInRange> searcher(me, unit, u_check);

    TypeContainerVisitor<SurgeCore::UnitLastSearcher<SurgeCore::MostHPMissingInRange>, GridTypeMapContainer >  grid_unit_searcher(searcher);
...
void SmartScript::DoFindFriendlyCC(std::list<Creature*>& _list, float range)
{
    ...
    SurgeCore::FriendlyCCedInRange u_check(me, range);
    SurgeCore::CreatureListSearcher<SurgeCore::FriendlyCCedInRange> searcher(me, _list, u_check);

    TypeContainerVisitor<SurgeCore::CreatureListSearcher<SurgeCore::FriendlyCCedInRange>, GridTypeMapContainer >  grid_creature_searcher(searcher);
...
void SmartScript::DoFindFriendlyMissingBuff(std::list<Creature*>& list, float range, uint32 spellid)
{
    ...
    SurgeCore::FriendlyMissingBuffInRange u_check(me, range, spellid);
    SurgeCore::CreatureListSearcher<SurgeCore::FriendlyMissingBuffInRange> searcher(me, list, u_check);

    TypeContainerVisitor<SurgeCore::CreatureListSearcher<SurgeCore::FriendlyMissingBuffInRange>, GridTypeMapContainer >  grid_creature_searcher(searcher);
