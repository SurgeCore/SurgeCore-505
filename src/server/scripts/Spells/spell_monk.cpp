...
class spell_monk_fists_of_fury_stun : public SpellScriptLoader
{
    public:
        ...
        class spell_monk_fists_of_fury_stun_SpellScript : public SpellScript
        {
            ...
            {
                targets.remove_if(SurgeCore::UnitAuraCheck(true, GetSpellInfo()->Id));
            }
        ...
        class spell_monk_expel_harm_SpellScript : public SpellScript
        {
            ...
            void HandleOnHit()
            {
                ...
                if (Player* _player = GetCaster()->ToPlayer())
                {
                    ...
                    SurgeCore::NearestAttackableUnitInObjectRangeCheck u_check(_player, _player, radius);
                    SurgeCore::UnitListSearcher<SurgeCore::NearestAttackableUnitInObjectRangeCheck> searcher(_player, targetList, u_check);
            ...
            void HandleOnHit()
            {
                ...
                if (Unit* _player = GetOriginalCaster()->ToPlayer())
                {
                    if (Unit* target = GetHitUnit())
                    {
                        ...
                        CellCoord p(SurgeCore::ComputeCellCoord(target->GetPositionX(), target->GetPositionY()));
                        Cell cell(p);
                        cell.SetNoCreate();

                        SurgeCore::AnyUnitInObjectRangeCheck u_check(_player, 20.0f);
                        SurgeCore::UnitListSearcher<SurgeCore::AnyUnitInObjectRangeCheck> searcher(_player, targetList, u_check);
                        ...
                        if (Unit* newTarget = sObjectAccessor->FindUnit(validTargets.front()))
                        {
                            ...
                            else
                            {
                                ...
                                alliesList.sort(SurgeCore::HealthPctOrderPred());
                        ...
                        if (!statueList.empty() && statueList.size() == 1)
                        {
                            ...
                            if (statue && (statue->isPet() || statue->isGuardian()))
                            {
                                if (statue->GetOwner() && statue->GetOwner()->GetGUID() == _plr->GetGUID())
                                {
                                    ...
                                    SurgeCore::Containers::RandomResizeList(targets, 1);
...
class spell_monk_thunder_focus_tea : public SpellScriptLoader
{
    public:
        ...
        class spell_monk_thunder_focus_tea_SpellScript : public SpellScript
        {
            ...
            {
                unitList.remove_if(SurgeCore::UnitAuraCheck(false, SPELL_MONK_RENEWING_MIST_HOT, GetCaster()->GetGUID()));
            }
        ...
        class spell_monk_renewing_mist_SpellScript : public SpellScript
        {
            ...
            void HandleOnHit()
            {
                if (Player* _player = GetCaster()->ToPlayer())
                {
                    ...
                    {
                        playerList.sort(SurgeCore::HealthPctOrderPred());
                        playerList.resize(1);
                    }
