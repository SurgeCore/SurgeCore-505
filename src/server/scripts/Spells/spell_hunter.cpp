    ...
    public:
        ...
        class spell_hun_glaive_toss_damage_SpellScript : public SpellScript
        {
            ...
            void CorrectDamageRange(std::list<WorldObject*>& targets)
            {
                ...
                SurgeCore::NearestAttackableUnitInObjectRangeCheck u_check(GetCaster(), GetCaster(), radius);
                SurgeCore::UnitListSearcher<SurgeCore::NearestAttackableUnitInObjectRangeCheck> searcher(GetCaster(), targetList, u_check);
    ...
    public:
        ...
        class spell_hun_lynx_rush_AuraScript : public AuraScript
        {
            ...
            void OnTick(constAuraEffectPtr aurEff)
            {
                ...
                SurgeCore::Containers::RandomResizeList(targetList, 1);
        ...
        class spell_hun_binding_shot_zone_AuraScript : public AuraScript
        {
            ...
            void OnUpdate(uint32 diff, AuraEffectPtr aurEff)
            {
                if (Unit* caster = GetCaster())
                {
                    ...
                    CellCoord p(SurgeCore::ComputeCellCoord(dynObj->GetPositionX(), dynObj->GetPositionY()));
                    Cell cell(p);
                    cell.SetNoCreate();

                    SurgeCore::AnyUnitInObjectRangeCheck u_check(dynObj, 15.0f);
                    SurgeCore::UnitListSearcher<SurgeCore::AnyUnitInObjectRangeCheck> searcher(dynObj, bindedList, u_check);

                    TypeContainerVisitor<SurgeCore::UnitListSearcher<SurgeCore::AnyUnitInObjectRangeCheck>, WorldTypeMapContainer > world_unit_searcher(searcher);
                    TypeContainerVisitor<SurgeCore::UnitListSearcher<SurgeCore::AnyUnitInObjectRangeCheck>, GridTypeMapContainer >  grid_unit_searcher(searcher);

                    cell.Visit(p, world_unit_searcher, *dynObj->GetMap(), *dynObj, 15.0f);
                    cell.Visit(p, grid_unit_searcher, *dynObj->GetMap(), *dynObj, 15.0f);

                    bindedList.remove_if(SurgeCore::UnitAuraCheck(false, GetSpellInfo()->Id, caster->GetGUID()));
...
class spell_hun_ancient_hysteria : public SpellScriptLoader
{
    public:
        ...
        class spell_hun_ancient_hysteria_SpellScript : public SpellScript
        {
            ...
            {
                targets.remove_if(SurgeCore::UnitAuraCheck(true, HUNTER_SPELL_INSANITY));
                targets.remove_if(SurgeCore::UnitAuraCheck(true, SPELL_SHAMAN_EXHAUSTED));
                targets.remove_if(SurgeCore::UnitAuraCheck(true, SPELL_SHAMAN_SATED));
                targets.remove_if(SurgeCore::UnitAuraCheck(true, SPELL_MAGE_TEMPORAL_DISPLACEMENT));
            }
    ...
    public:
        ...
        class spell_hun_pet_carrion_feeder_SpellScript : public SpellScript
        {
            ...
            SpellCastResult CheckIfCorpseNear()
            {
                ...
                SurgeCore::AnyDeadUnitSpellTargetInRangeCheck check(caster, max_range, GetSpellInfo(), TARGET_CHECK_ENEMY);
                SurgeCore::WorldObjectSearcher<SurgeCore::AnyDeadUnitSpellTargetInRangeCheck> searcher(caster, result, check);
