            ...
            void HandleOnHit()
            {
                if (Unit* caster = GetCaster())
                {
                    ...
                    if (Unit* target = GetHitUnit())
                    {
                        ...
                        SurgeCore::NearestAttackableUnitInObjectRangeCheck u_check(caster, caster, radius);
                        SurgeCore::UnitListSearcher<SurgeCore::NearestAttackableUnitInObjectRangeCheck> searcher(caster, targetList, u_check);
                ...
                if (Player* _player = GetCaster()->ToPlayer())
                {
                    if (Unit* target = GetHitUnit())
                    {
                        ...
                        if (chargeCount)
                        {
                            ...
                            SurgeCore::Containers::RandomResizeList(targetList, chargeCount);
                ...
                if (Player* _player = GetCaster()->ToPlayer())
                {
                    if (Unit* target = GetHitUnit())
                    {
                        ...
                        if (targetList.size() > 2)
                            SurgeCore::Containers::RandomResizeList(targetList, 2);
