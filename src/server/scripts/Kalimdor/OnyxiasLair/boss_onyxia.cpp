                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_BELLOWING_ROAR: // Phase PHASE_END
                        {
                            ...
                            SurgeCore::GameObjectInRangeCheck check(me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), 15);
                            SurgeCore::GameObjectLastSearcher<SurgeCore::GameObjectInRangeCheck> searcher(me, Floor, check);
