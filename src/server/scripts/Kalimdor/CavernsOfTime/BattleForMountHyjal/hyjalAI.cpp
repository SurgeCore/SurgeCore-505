...
{
    CellCoord pair(SurgeCore::ComputeCellCoord(x, y));
    Cell cell(pair);
    cell.SetNoCreate();

    // First get all creatures.
    std::list<Creature*> creatures;
    SurgeCore::AllFriendlyCreaturesInGrid creature_check(me);
    SurgeCore::CreatureListSearcher<SurgeCore::AllFriendlyCreaturesInGrid> creature_searcher(me, creatures, creature_check);

    TypeContainerVisitor <SurgeCore::CreatureListSearcher<SurgeCore::AllFriendlyCreaturesInGrid>, GridTypeMapContainer> creature_visitor(creature_searcher);
...
void hyjalAI::RespawnNearPos(float x, float y)
{
    ...
    SurgeCore::RespawnDo u_do;
    SurgeCore::WorldObjectWorker<SurgeCore::RespawnDo> worker(me, u_do);
    TypeContainerVisitor<SurgeCore::WorldObjectWorker<SurgeCore::RespawnDo>, GridTypeMapContainer > obj_worker(worker);
...
void hyjalAI::WaypointReached(uint32 waypointId)
{
    if (waypointId == 1 || (waypointId == 0 && me->GetEntry() == THRALL))
    {
        ...
        TypeContainerVisitor
            <SurgeCore::CreatureListSearcher<SurgeCore::AllFriendlyCreaturesInGrid>,
